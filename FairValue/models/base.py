    ...
    def check_non_negative(
        ...
        for value in values:
            if value is not None and not np.isnan(value) and value < 0:
                raise ValueError(f"All values must be non-negative floats, None, or np.nan. Got {type(value)}")
        return values
...
class Ints(BaseModel):
    ...
    data: List[int] = Field(
        ...,
        description="A list of int values.",
    )
...
class Strs(BaseModel):
    ...
    data: List[str] = Field(
        ...,
        description="A list of str values.",
    )
