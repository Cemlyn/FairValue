...
class Stock:
    ...
    def plot_financial(self, financial_id: str = "capex") -> axes:
        ...
        if financial_id == "capex":
            ...
            pd.Series(capex_series, index=self.financial_dates).sort_index().plot.bar(
                ax=axes
            )
    ...
    def run_scenario_from_params(self, scenario_params: ScenarioParams = None) -> float:
        ...
        if scenario_params is None:
            ...
            scenario_params = ScenarioParams(
                free_cashflows=Floats(data=[fcf for _ in range(years)]),
        ...
        intrinsic_value = calc_intrinsic_value(
            scenario_params.free_cashflows,
    ...
    def __init__(self, filepath):
        ...
        for ticker_id, ticker_df in tqdm.tqdm(df.groupby("ticker")):
            ...
            try:
                ...
                self.stocks[ticker_id] = Stock(
                    ticker=ticker_id, ticker_financials=ticker_financials
                )
            except ValidationError:
                warn_string = (
                    f'ticker "{ticker_id} failed validation". Skipping ticker.'
                )
...
def calc_intrinsic_value(
    ...
    last_fcf = max(
        free_cashflows[-1] * (1 + growth[-1]), 0
    )  # Last FCF grows by the last year's growth rate
